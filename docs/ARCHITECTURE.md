...
- `DeathLifecycle.cs`：死亡、复活与阵容事务；`CanReceivePredictedPowers` 按「个体是否仍在战斗里」判 Power 可施加——实机在击杀当时就 `RemoveCreature`，而死亡效果要到 `ApplyEnemyDeathPowers` 才清扫，这个窗口里施加给已离场个体的 Power 是空操作；
