...
internal static partial class PredictionCoverage
{
    ...
    private static (string SourceId, string Method, string Reason, bool Compensated) DescribeGap(CombatPredictionRiskEntry entry)
    {
        ...
        bool compensated = source switch
        {
            ...
            // 第三方「死亡后保留尸体并复活／重生」的 Power（登记见 RegisterRevivePower／
            // RegisterRespawnPower）与原版 ReattachPower／AdaptablePower 走同一条处理：
            // 语义在死亡阶段入口实现，AfterDeath 这条重写因此算已补偿。
            PowerModel power when method == "AfterDeath"
                && (ThirdPartyAdapterRegistry.IsRevivePowerName(power.GetType().Name)
                    || ThirdPartyAdapterRegistry.IsRespawnPowerName(power.GetType().Name)) => true,
