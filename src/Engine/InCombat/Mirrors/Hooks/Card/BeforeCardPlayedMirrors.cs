...
internal static class BeforeCardPlayedMirrors
{
    ...
    /// <summary>
    /// 第三方适配 Mod 按运行时类型登记 <c>AbstractModel.BeforeCardPlayed</c> 的预测实现。
    /// </summary>
    /// <remarks>
    /// 登记须在任何根捕获或首次分发之前完成，之后明确拒绝；未登记的第三方重写会在分发时记一条
    /// <c>MethodNotMirrored</c>（见 docs/THIRD_PARTY_ADAPTERS.md §2.13）。
    /// </remarks>
    public static void Register(Type modelType, Action<AbstractModel, BeforeCardPlayedMirrorContext> handler)
    {
        ArgumentNullException.ThrowIfNull(handler);
        if (modelType.IsAbstract)
            throw new ArgumentException("出牌前镜像需要具体运行时类型。", nameof(modelType));
        lock (RegistrationLock)
        {
            if (_sealed)
                throw new InvalidOperationException("BeforeCardPlayed 镜像必须在根捕获或首次分发之前登记。");
